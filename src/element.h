...
namespace edb
{
	...
	/// \sa edb::Element::xrayCoefficients()
...
namespace std
{
	/// Feed the string representation of a database element into a std::stream.
	ostream& operator<<(ostream& o, const edb::Element& e);
	/// Comparison of complex numbers by length (Euclidian Norm).
	bool operator<(const edb::complex& a, const edb::complex& b);
}
